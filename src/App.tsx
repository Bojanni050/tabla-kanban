...
function App() {
  ...
  // Template flow: apply a board template's lists, labels, swimlanes and card
  // types in one atomic server operation. The server persists everything in a
  // single transaction (existing names are reused untouched), then broadcasts
  // the usual per-item realtime events. Local state merges the created rows;
  // no reload, no cards touched.
  const handleApplyBoardTemplate = async (
    boardId: string,
    snapshot: BoardTemplateSnapshot
  ): Promise<BoardTemplateResult | null> => {
    try {
      const result = await api.applyBoardTemplate(boardId, snapshot);
      const byPosition = (a: { position: number }, b: { position: number }) => a.position - b.position;
      setBoard((prev) => {
        if (!prev || prev.id !== boardId) return prev;
        return {
          ...prev,
          lists: [...prev.lists, ...result.lists].sort(byPosition),
          labels: [...(prev.labels || []), ...result.labels.created],
          swimlanes: [...(prev.swimlanes || []), ...result.swimlanes.created].sort(byPosition),
          cardTypes: [...(prev.cardTypes || []), ...result.cardTypes.created].sort(byPosition),
        };
      });
      return result;
    } catch {
